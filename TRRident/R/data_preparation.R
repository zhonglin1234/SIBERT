#' @title Data Preparation Functions for TRRident
...
rescale_loc <- function(loc, pt, aa = 1) {
  loc.rsc <- loc
  loc.rsc[names(pt), ] <- loc[names(pt), ] * aa
  return(loc.rsc)
}
...
assign_subcluster <- function(cut.t, spots, spot.sdist, nr, r.unit, spot.pt.dist) {
  ...
  # Additional processing for larger clusters
  return(tmp.list)
}

meta_subclusters <- function(sub.clusters, loc, pt, meta_dat, keep.ids) {
  meta_dat_cell <- meta_dat[keep.ids, ]
  loc.cts <- do.call(rbind, lapply(sub.clusters, function(x)
    colSums(loc[x, ]) / length(x)))
  rownames(loc.cts) <- paste0('cts', 1:nrow(loc.cts))
  colnames(loc.cts) <- c('x', 'y')

  pt.cts <- unlist(lapply(sub.clusters, function(x) mean(pt[x])))
  n.spots <- unlist(lapply(sub.clusters, length))

  meta.cts <- data.frame(loc.cts, pt = pt.cts, n.spots = n.spots)
  return(meta.cts)
}
